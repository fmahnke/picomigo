...
try:
    from unittest import TestResult, TestRunner, TestSuite
except ImportError:
    print(
        "Error: This must be used from an installed copy of unittest-discover which will"
...
def _run_test_module(
    runner: TestRunner, module_name: str, *extra_paths: list[str]
):
...
def discover_main():
    ...
    if len(sys.argv) == 1 or (len(sys.argv) >= 2 and _dirname_filename_no_ext(
            sys.argv[0])[1] == "unittest" and sys.argv[1] == "discover"):
